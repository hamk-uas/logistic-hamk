    ...
    if not default_num_vehicles:
        ...
        if uploaded_file2_1 is not None:
            ...
            if address_option2 == "Single Column":
                ...
                st.text(column_names)
                address_cols_input = st.text_input("Enter all column names(case-sensitive) for the address (comma-separated). Address in order: Street,City,Country,", placeholder="like: Column1,column2,column3...", key="1")  # Provide example inside placeholder
                address_cols = [col.strip() for col in address_cols_input.split(",")]
                address_col = None  
        ...
        if uploaded_file2_2 is not None:
            ...
            if address_option2 == "Single Column":
                ...
                st.text(column_names)
                address_cols_input = st.text_input("Enter all column names(case-sensitive) for the address (comma-separated). Address in order: Street,City,Country,", placeholder="like: Column1,column2,column3...", key="2")  # Provide an example inside the placeholder
                address_cols = [col.strip() for col in address_cols_input.split(",")]
                address_col = None  
